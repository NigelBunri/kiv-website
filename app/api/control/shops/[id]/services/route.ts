...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const upstream = await fetch(`${kisApiBase()}/api/v1/commerce/shop-services/?shop=${encodeURIComponent(id)}`, {
      headers: authHeaders(session), cache: "no-store", signal: AbortSignal.timeout(15_000),
    });
    const data = await upstream.json().catch(() => ({}));
    const response = NextResponse.json(upstream.ok ? { success: true, data } : { success: false, message: "Unable to load services." }, { status: upstream.status });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const upstream = await fetch(`${kisApiBase()}/api/v1/commerce/shop-services/`, {
      method: "POST",
    ...
    const response = NextResponse.json(
      upstream.ok ? { success: true, data } : { success: false, message: data?.detail || firstError(data) || "Unable to create service.", errors: data },
